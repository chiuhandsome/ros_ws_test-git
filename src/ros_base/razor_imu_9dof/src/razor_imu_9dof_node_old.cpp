...
bool Razor_Imu::CommPortIsOpened()
{
  ...
  try
  {
    ...
    for(int i = 0 ;i < PORT_OPEN_RETRY ; i++)
    {
       if (imu_serial_Ptr->()) {
         portIsOpened = true;
         break ;
...
void Razor_Imu::parse_bytes(string& vdata)
{
  ...
  if ((old_data_count < 20) && (CB_status_ == S_DATA))
  {
    {
      std::unique_lock<std::mutex> lock(mx);
      old_data_count++;
...
void Razor_Imu::writeCalibrationData()
{
  ...
  ros::V_string cal = imu_serial_Ptr->readlines();
  std::string calib_data_print = "Printing set calibration values:sync\r\n";
  for (auto line : cal)
  {
    calib_data_print += line;
  }
