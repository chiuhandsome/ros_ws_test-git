...
#include <open_auto_dock_msgs/Cloud.h>
#include <std_msgs/String.h>
////#include <robot_database_bridge/Cloud_cmd.h>
// #include <robot_database_bridge/Cloud.h>
...
class AutoDocking
{
  ...
  typedef actionlib::SimpleActionServer<open_auto_dock_msgs::DockAction> dock_server_t;
  typedef actionlib::SimpleActionServer<open_auto_dock_msgs::UndockAction> undock_server_t;
  ////using SRV_DB = robot_database_bridge::Cloud_cmd;
  // using MSG_DB = robot_database_bridge::Cloud;
  ////using SRV_PAR = botcmd_msgs::bot_do_cmd;
public:
  /**
...
private:
  ...
  ros::ServiceClient client_;
	ros::Subscriber sub_;
  //bool dynamic_param(SRV_PAR::Request &req,SRV_PAR::Response &res);
  //bool call_database(int cmd_id,std::string key,std::vector<float> &data);
  /**
   ...
   */
  void dockCallback(const open_auto_dock_msgs::DockGoalConstPtr& goal);
   ...
   */
  bool continueDocking(open_auto_dock_msgs::DockResult& result,
  ...
  double auto_backup_dist_factor;
  double auto_dock_timeout ;
  double auto_undock_timeout ;

  bool bdockfinishflag ;
  bool predockflag ;
  void predockConditions(double shif_x,double shift_y);
  void shiftGoal(geometry_msgs::PoseStamped& pose,double shift_x,double shift_y);
  double shift_x,shift_y,shift_mark,rate,predock_threshold,undock_rotate;
  int reverse;
};
