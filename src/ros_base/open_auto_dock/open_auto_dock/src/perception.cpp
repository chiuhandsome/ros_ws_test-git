  ...
  nh_(nh)
{
    ...
    if (debug_)
    {
        debug_points_ = nh_.advertise<sensor_msgs::PointCloud2>("dock_points", 10);
    }

    // Init base_scan only after publishers are created
    scan_sub_ = nh_.subscribe(lidar_scan_name, 1, &DockPerception::callback, this);
    //ROS_INFO_NAMED("perception","Dock perception initialized");
    //-- 增加發佈點雲的測試 ----
    //if (debug_)
        testcloud_points_ = nh_.advertise<sensor_msgs::PointCloud2>("testcloud_points", 10);
}
//-----------------------------------------------------------------------------
void DockPerception::set_lidar_name(std::string name){
  lidar_scan_name = name;
  scan_sub_ = nh_.subscribe(lidar_scan_name, 1, &DockPerception::callback, this);
  ROS_INFO("set lidar_name[%s]",lidar_scan_name.c_str());
}
//---- Create ideal cloud  ----------------------------------------------------
//-- 建立 理想雲資料 的比對資料 -----------------------------------------------------------
double DockPerception::create_dynamic_cloud(double left,double middle,double right,double bottom,double depth,double unit){ //default unit=0.005
  ideal_cloud_.clear();
  double half = (left+middle+right)/2 ;
  double bc = -half + right + middle/2 ;
  for (double y = -half; y <= half; y += unit){
    geometry_msgs::Point p;
    p.z = 0;
    p.y = y;
    if(fabs(y-bc)< bottom/2){
      p.x = depth;
    }else if(fabs(y-bc)< middle/2){
      p.x = depth*(1-(fabs(y-bc)-bottom/2)/((middle-bottom)/2));
    }else{
      p.x = 0;
    }
    ideal_cloud_.push_back(p);
  }
  ROS_INFO("create dynamic cloud l%.3f m%.3f r%.3f b%.3f d%.3f",left,middle,right,bottom,depth);
  return bc;
}
...
void DockPerception::Create_idealcloud()
{
    ...
    create_dynamic_cloud(dock_left,dock_middle,dock_right,dock_bottom,dock_depth);
}
//-----------------------------------------------------------------------------
...
void DockPerception::Read_CntParameters()
{
    ...
    //dynamic dock
    if(!pnh.getParam("dock_left", dock_left))
        dock_left = .33;
    if(!pnh.getParam("dock_middle", dock_middle))
        dock_middle = .20;
    if(!pnh.getParam("dock_right", dock_right))
        dock_right = .065;
    if(!pnh.getParam("dock_bottom", dock_bottom))
        dock_bottom = .08;
    if(!pnh.getParam("dock_depth", dock_depth))
        dock_depth = .04;
...
void DockPerception::callback(const sensor_msgs::LaserScanConstPtr& scan)
{
        ...
        (dock_.pose.orientation.z == 0.0 && dock_.pose.orientation.w == 0.0))
    {
        ...
        //for (size_t i = front_lidar_angel_min; i < front_lidar_angel_max; i++)
        for (size_t i = 0; i < scan->ranges.size(); i++)
        {
            //if (i >= scan->ranges.size()){i -= scan->ranges.size();}
    ...
    if(candidates.empty() && defeat.size()){
      ...
      z1 = {defeat.top()->points[0].x,defeat.top()->points[0].y};
      z2 = {defeat.top()->points[last].x,defeat.top()->points[last].y};
      ROS_INFO("candidates[%lu->0]defeat.top[%.2f,%.2f]->[%.2f,%.2f]dist[%.2f]width[%.2f]",processor.getClusters().size(),real(z1),imag(z1),real(z2),imag(z2),defeat.top()->dist,abs(z1-z2));
    }
