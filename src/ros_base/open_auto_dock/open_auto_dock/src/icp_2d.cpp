...
namespace icp_2d
{
double corr_max_dist_check = 1.0 ;
...
//-----------------------------------------------------------------------------  
//-- 計算點集 source 及 target 的對應矩陣（點集），此矩陣為 以 sorce 中的每一點為 base 
//-- 在target 所有點中找到最近的點，將其值存入 correspondences 中
//-- 最小值不可大於 設定的參數
                            ...
                            //double corr_min_dist_check)
{
  ...
  {
    double d = corr_max_dist_check ;//1.0;
    size_t best = 0;
...
//-- 算出 source 及 target 間的 Transform （平移及旋轉） 
//-- 1.以 thetaFromQuaternion(t.rotation)為旋轉角度，t.translation.x,t.translation.y
//--   為平移中心，將 source -〉source_t 
//-- 2.求 source_t 及 target 的質心，計算兩質心的 offset 並重新 update 
//--   t.translation.x,t.translation.y 的值
//-- 3.找出 source_t 及 target 各自最大的 eigenvector，利用這兩個向量就可一旋轉矩陣，并求得
//--   其夾角，可推得 t.rotation.z，t.rotation.w
//--   （1）找 covariance matrix ：求取自身矩陣和其質心點的差（Ps 及 Pt） 
//--        Ms = Ps * Ps.transpose(); Mt = Pt * Pt.transpose();  
//--   （2）利用 Ms 及 Mt 找出所有其相對的 eigenvectorsource_ts 及 eigenvalues 
//--   （3）對 eigenvalues 的大小來對 eigenvectors 排序 A 及 B （學理上，找出最大，進行降階）
//--   （4）求⻆度 ： R = B * A.transpose();   theta += atan2(R(1, 0), R(0, 0));   
//--        t.rotation.z = sin(theta / 2.0);  t.rotation.w = cos(theta / 2.0);
              ...
              geometry_msgs::Transform& t)
{
  ...
  // Compute P for source (matrix for float)
...
//-- 1.以 thetaFromQuaternion(t.rotation)為旋轉角度，t.translation.x,t.translation.y
//--   為平移中心，將 source -〉source_t 
//-- 2.找 Correspondences matrix ：corr 詳 computeCorrespondences 函式說明 
//-- 3.求 corr 及 target 的質心，計算兩質心的 offset 並重新 update 
//--   t.translation.x,t.translation.y 的值
//-- 4.求得變異數矩陣 M，利用 svd(M, Eigen::ComputeThinU | Eigen::ComputeThinV) 求得其夾角
//--   可推得 t.rotation.z，t.rotation.w
//--   （1）R = svd.matrixV() * svd.matrixU().transpose(); 
//--   （2）求⻆度 ： R = B * A.transpose();   theta += atan2(R(1, 0), R(0, 0));   
//--        t.rotation.z = sin(theta / 2.0);  t.rotation.w = cos(theta / 2.0);
...
//----------------------------------------------------------------------------- 
//-- 取得均方根誤差 root-mean-square deviation
//-- 1.找 Correspondences matrix ：corr 詳 computeCorrespondences 函式說明 
//-- 2.取得均方根值
...
//-- 1. alignPCA 取得 Transform t
//-- 2. 進入執行 疊代循環執行 （max_iterations）
//--    (1) alignSVD =>正確，取得 t
//--    (2) 以 thetaFromQuaternion(t.rotation)為旋轉角度，t.translation.x,t.translation.y
//--        為平移中心，將 source -〉source_t 
//--    (3) 取得均方根值 source_t/target
//--        如果 (fabs(prev_rmsd - rmsd) < min_delta_rmsd)，目標己達成
//-- 3. 在 item 2 的廻圈中未達目標，執行 2.(2)及2.(3),回傳 均方根值 資料  
                ...
                //double min_delta_rmsd)
{
  ...
  //ROS_INFO("fit:alignICP - run alignPCA");
  ...
  //ROS_INFO("fit:alignICP - run alignSVD");
...
void icp_cntparameter_Set(size_t icp_max_iterations,double icp_min_delta_rmsd)
{
  max_iterations = icp_max_iterations ;
  min_delta_rmsd = icp_min_delta_rmsd ;
}

void corr_max_dist_checkSet(double icp_corr_max_dist_check)
{
  corr_max_dist_check = icp_corr_max_dist_check ;
}
